...
export async function getChatScrollByPartyIdandPageId(req, res) {
  ...
  const { allCount } = await chatRespository.getChatAllCountByPartyId(
    req.partyId
  );

  const pageStartId = parseInt((pageId - 1) * config.page.scrollLimit);
  ...
  console.log(
    `숫자 총갯구 ${pageStartId + parseInt(config.page.scrollLimit)} ${allCount}`
  );
  console.log(pageStartId + parseInt(config.page.scrollLimit) < allCount);
  chats.sort((a, b) => a.id - b.id);
  const sendData = {
    chats,
    hasMore:
      pageStartId + parseInt(config.page.scrollLimit) < allCount ? true : false,
  };
