...
export async function partyReady(req, res) {
  ...
  if (user.owner) {
    const party = await partyRepository.startPartyByPartyId(user.partyId);
    console.log(party);
    if (party) {
      const mambers = await userRepository.findUsersByPartyId(user.partyId);
      party['members'] = mambers;
