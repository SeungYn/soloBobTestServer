...
export async function getChatByPartyIdandpageId(partyId, pageId) {
  console.log(partyId, pageId);
  return db
    .execute('SELECT * FROM chat WHERE room = ? order by id DESC limit ?, ?', [
      partyId,
      pageId,
      config.page.scrollLimit,
    ])
    .then((result) => result[0]);
}

export async function getChatAllCountByPartyId(partyId) {
  return db
    .execute('SELECT count(id) as allCount FROM chat WHERE room = ?', [partyId])
    .then((result) => result[0][0]);
}
